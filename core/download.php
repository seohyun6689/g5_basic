...
if($board['bo_download_point'] < 0 && $is_guest)
    alert(_(theme_t721), G5_BBS_URL.'/login.php?wr_id='.$wr_id.'&amp;'.$qstr.'&amp;url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id));

$sql = " select bf_source, bf_file from {$g5['board_file_table']} where bo_table = '$bo_table' and wr_id = '$wr_id' and bf_no = '$no' ";
$file = sql_fetch($sql);
if (!$file['bf_file'])
    alert_close(_(core_a38));

// JavaScript 불가일 때
if($js != 'on' && $board['bo_download_point'] < 0) {
    $msg = $file['bf_source']. _(core_a39, number_format($board['bo_download_point']));
...
if ($member['mb_level'] < $board['bo_download_level']) {
    $alert_msg = _(core_a40);
    if ($member['mb_id'])
        alert($alert_msg);
    else
        alert($alert_msg. _(core_a41), G5_BBS_URL.'/login.php?wr_id='.$wr_id.'&amp;'.$qstr.'&amp;url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id));
}
...
if (!get_session($ss_name))
{
    ...
    else if ($board['bo_download_level'] >= 1) // 회원이상 다운로드가 가능하다면
    {
        ...
        if ($member['mb_point'] + $board['bo_download_point'] < 0)
            alert(_(core_a43, array(number_format($member['mb_point']), number_format($board['bo_download_point']))));
