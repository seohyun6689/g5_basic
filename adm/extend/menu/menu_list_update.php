...
// 이전 메뉴정보 삭제
$sql = " delete from {$g5['menu_table']} where me_group = '{$me_group}'";
sql_query($sql);
...
for ($i=0; $i<$count; $i++)
{
    ...
    $me_sub_name = $_POST['me_sub_name'][$i];
    ...
    if($group_code == $code) {
        $sql = " select MAX(SUBSTRING(me_code,3,2)) as max_me_code
                    from {$g5['menu_table']}
                    where me_group = '{$me_group}' AND SUBSTRING(me_code,1,2) = '$primary_code' ";
        $row = sql_fetch($sql);
        ...
        $sql = " select MAX(SUBSTRING(me_code,1,2)) as max_me_code
                    from {$g5['menu_table']}
                    where me_group = '{$me_group}' AND LENGTH(me_code) = '2' ";
        $row = sql_fetch($sql);
    ...
    $sql = " insert into {$g5['menu_table']}
                set me_group        = '$me_group',
                	me_code         = '$me_code',
                    me_name         = '$me_name',
                    me_sub_name     = '$me_sub_name',
                    me_link         = '$me_link',
